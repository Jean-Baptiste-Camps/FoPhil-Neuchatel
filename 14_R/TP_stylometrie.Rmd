...
La stylométrie a développé ses propres mesures de distance. Une d'entre elles, parmi les plus récentes, est la métrique 'min max' (Koppel & Winter, 2014). D'emploi (un peu plus) courant est la similarité cosinus.

```{r, fig.width=20, fig.height=10, dpi=45}
library(stylo)
maCAH = agnes(dist.cosine(t(theatreTraite)), method="ward")
plot(maCAH)
```
...
Commençons par calculer les distances entre nos individus (nous choisissons une nouvelle fois la distance de Manhattan de préférence à la distance euclidienne).

```{r}
distance = dist(t(theatreTraite), method = "manhattan")
```

et nous «ajustons» sur deux dimensions,
```{r}
fit = cmdscale(distance,eig=TRUE, k=2) # k est le nombre de dimensions souhaité
```
